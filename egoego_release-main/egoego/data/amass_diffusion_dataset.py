...
def run_smpl_model(root_trans, aa_rot_rep, betas, gender, bm_dict):
    ...
    for gender_name in gender_names:
        ...
        # reconstruct SMPL
        #对当前性别的样本使用对应的 SMPL 模型进行预测，得到预测的关节和顶点位置，
        # 并将结果存储在 pred_joints 和 pred_verts 列表中
...
#从 SMPL 模型文件中获取关节的父子关系（即骨骼树结构），并返回一个表示父节点索引的数组。
...
#将关节的旋转矩阵转换为四元数，并通过逆运动学计算出相对于父关节的局部旋转姿态
...
#该函数实现了前向运动学的计算，给定局部旋转矩阵和位置，通过关节的父子关系逐步计算出每个关节在全局空间中的位置和旋转
...
#该方法实现了 SMPL 模型的前向运动学计算，使用给定的根关节位移和各个关节的局部旋转来计算每个关节在全局空间中的位置和旋转
...
#从原始数据中筛选出长度大于等于指定窗口大小的序列，同时记录最长的序列长度，并返回符合条件的序列集合
